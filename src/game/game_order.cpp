...
bool Game_Order::is_valid () const {
    ...
    if (type == Type::BUILD_CITY) {
        const Region& region = Game::get_region(Game::get_chunk(Coords<uint32_t>(Tile::get_chunk_x(coords.x),
                                                                                 Tile::get_chunk_y(
                                                                                     coords.y))).get_parent_region());
...
void Game_Order::execute () {
    if (is_valid()) {
        if (type == Type::BUILD_CITY) {
            ...
            Game::new_tiles.emplace(std::piecewise_construct, std::forward_as_tuple(building_tile_coords),
                                    std::forward_as_tuple(civilization_index, get_tile_type()));
        } else if (type == Type::REPOPULATE_CITY) {
    ...
    if (!Game::is_mouse_over_minimap()) {
        ...
        if (type == Type::BUILD_CITY) {
            ...
            Render::render_rectangle_empty(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                           ...
                                           tile_size * Game_Manager::camera_zoom, tile_size * Game_Manager::camera_zoom,
                                           1.0, color,
                                           Game_Constants::RENDER_BUILDING_COLOR_BORDER * Game_Manager::camera_zoom);

            Render::render_texture(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                   y * Game_Manager::camera_zoom - Game_Manager::camera.y,
                                   Image_Manager::get_image("tile_" + Tile::get_type_string(get_tile_type())), 0.75,
            ...
            if (!valid) {
                ...
                double message_x = x + tile_size / 2.0 - ((message.length() * font->spacing_x) / 2.0) /
                                   Game_Manager::camera_zoom;

                font->show(message_x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                           (y + tile_size) * Game_Manager::camera_zoom - Game_Manager::camera.y, message, "ui_white");
            }
        }
    }
}
