...
#include <collision/int/int_collision.h>
#include <engine.h>
...
Collision_Rect<int32_t> Person::get_sight_box () const {
    ...
    return Collision_Rect<int32_t>(box.center_x() - sight_range, box.center_y() - sight_range, sight_range * 2,
                                   sight_range * 2);
}
...
bool Person::is_goal_valid () const {
    if (goal.is_gather() && has_inventory_space() && Game::tile_exists(goal.get_coords_tiles())) {
        ...
        if (tile.is_alive() && tile.get_type() == Tile::Type::BUILDING_UNFINISHED &&
            tile.get_parent() == get_parent_civilization() && has_build_material()) {
            return true;
        } else {
            return false;
        }
        ...
        // If the tile's type is a city building, its parent is a city
        if (tile.is_alive() && tile.needs_repair() && tile.get_type() == Tile::Type::BUILDING_CITY &&
            tile.get_parent() == get_parent_city() && has_repair_material()) {
            return true;
        } else {
            return false;
        }
...
Coords<int32_t> Person::get_goal_coords () const {
    ...
    if (goal.is_gather() || goal.is_empty_inventory() || goal.is_eat_at_home() || goal.is_forage() ||
        goal.is_attack_building_melee() || goal.is_build() || goal.is_repair()) {
...
int32_t Person::get_angle_to_goal () const {
    ...
    if (goal_coords.x >= 0) {
        return Int_Collision::get_angle_to_rect(Collision_Rect<int32_t>(box.x, box.y, 0, 0),
                                                Collision_Rect<int32_t>(goal_coords.x, goal_coords.y, 0, 0));
    } else {
        return 0;
    }
}
...
bool Person::is_at_home () const {
    ...
    if (Int_Math::get_distance_between_points_no_sqrt(box.get_center(),
                                                      Coords<int32_t>(city.get_center_x(),
                                                                      city.get_center_y())) <= get_goal_range()) {
        return true;
    } else {
        return false;
    }
    ...
    if (is_goal_valid()) {
        if (goal.is_gather()) {
            ...
            if (our_materials >= Game_Constants::COST_BUILD) {
                ...
                Game::remove_civilization_item(get_parent_civilization(), Inventory::Item_Type::TREE,
                                               Game_Constants::COST_BUILD - our_materials);
            }

            if (Game::get_selection().type == Game_Selection::Type::UNFINISHED_BUILDING &&
                Game::get_selection().tile_coordinates == goal.get_coords_tiles()) {
            ...
            if (our_materials >= Game_Constants::COST_REPAIR) {
                ...
                Game::remove_civilization_item(get_parent_civilization(), Inventory::Item_Type::TREE,
                                               Game_Constants::COST_REPAIR - our_materials);
            }
...
void Person::write_info_string (string& text) const {
    ...
    text += "Health: " + Strings::num_to_string(health) + "/" + Strings::num_to_string(get_health_max()) +
            hunger_state + "\n";
    ...
    for (size_t i = 0; i < item_types.size(); i++) {
        text += Inventory::get_item_type_string(item_types[i]) + ": " +
                Strings::num_to_string(get_item_count(item_types[i])) + "\n";
    }
}
...
void Person::animate () {
    if (is_alive()) {
        if (velocity.magnitude != 0) {
            ...
            int animation_speed = (double) Game_Constants::PERSON_MOVE_ANIMATION_SPEED_MAX /
                                  (Math::abs((double) velocity.magnitude) / (double) Game_Constants::PERSON_MAX_SPEED);
...
void Person::render (bool selected) const {
    if (is_alive()) {
        ...
        if (Collision::check_rect(box_render * Game_Manager::camera_zoom, Game_Manager::camera)) {
            ...
            if (facing == "left") {
                sprite_body_left.render(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                        y * Game_Manager::camera_zoom - Game_Manager::camera.y, 1.0,
                                        Game_Manager::camera_zoom, Game_Manager::camera_zoom);
                sprite_shirt_left.render(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                         y * Game_Manager::camera_zoom - Game_Manager::camera.y, 1.0,
                                         Game_Manager::camera_zoom, Game_Manager::camera_zoom, 0.0, color);
            } else if (facing == "right") {
                sprite_body_right.render(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                         y * Game_Manager::camera_zoom - Game_Manager::camera.y, 1.0,
                                         Game_Manager::camera_zoom, Game_Manager::camera_zoom);
                sprite_shirt_right.render(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                          y * Game_Manager::camera_zoom - Game_Manager::camera.y, 1.0,
                                          Game_Manager::camera_zoom, Game_Manager::camera_zoom, 0.0, color);
            } else if (facing == "up") {
                sprite_body_up.render(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                      y * Game_Manager::camera_zoom - Game_Manager::camera.y, 1.0,
                                      Game_Manager::camera_zoom, Game_Manager::camera_zoom);
                sprite_shirt_up.render(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                       y * Game_Manager::camera_zoom - Game_Manager::camera.y, 1.0,
                                       Game_Manager::camera_zoom, Game_Manager::camera_zoom, 0.0, color);
            } else if (facing == "down") {
                sprite_body_down.render(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                        y * Game_Manager::camera_zoom - Game_Manager::camera.y, 1.0,
                                        Game_Manager::camera_zoom, Game_Manager::camera_zoom);
                sprite_shirt_down.render(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                         y * Game_Manager::camera_zoom - Game_Manager::camera.y, 1.0,
                                         Game_Manager::camera_zoom, Game_Manager::camera_zoom, 0.0, color);
            }
            ...
            Render::render_rectangle((x - back_thickness) * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                     ...
                                     (bar_height + back_thickness * 2.0) * Game_Manager::camera_zoom, 1.0, "ui_black");
            ...
            if (is_hungry()) {
                Render::render_texture((x + max_bar_width + back_thickness * 2.0) * Game_Manager::camera_zoom -
                                       Game_Manager::camera.x,
                                       (y - bar_height - back_thickness * 4.0) * Game_Manager::camera_zoom -
                                       Game_Manager::camera.y, Image_Manager::get_image("icon_person_hungry"), 1.0,
                                       Game_Manager::camera_zoom, Game_Manager::camera_zoom);
            } else if (is_starving()) {
                Render::render_texture((x + max_bar_width + back_thickness * 2.0) * Game_Manager::camera_zoom -
                                       Game_Manager::camera.x,
                                       (y - bar_height - back_thickness * 4.0) * Game_Manager::camera_zoom -
                                       Game_Manager::camera.y, Image_Manager::get_image("icon_person_starving"), 1.0,
                                       Game_Manager::camera_zoom, Game_Manager::camera_zoom);
            }
            ...
            if (selected) {
                Render::render_rectangle_empty((x - Game_Constants::RENDER_SELECTION_SIZE) * Game_Manager::camera_zoom -
                                               ...
                                               Game_Manager::camera.y,
                                               ((double) box.w + Game_Constants::RENDER_SELECTION_SIZE * 2.0) *
                                               Game_Manager::camera_zoom,
                                               ((double) box.h + Game_Constants::RENDER_SELECTION_SIZE * 2.0) *
                ...
                if (has_goal()) {
                    Render::render_texture(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                           (y - bar_height - back_thickness * 4.0 - 16) * Game_Manager::camera_zoom -
                                           Game_Manager::camera.y, Image_Manager::get_image("arrow"), 1.0,
                                           Game_Manager::camera_zoom, Game_Manager::camera_zoom, get_angle_to_goal(),
                                           "selection");
                }
            }
