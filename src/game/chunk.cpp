...
uint32_t& Tile_Counts::get_tile_reference (Tile::Type tile_type) {
    if (tile_type == Tile::Type::WHEAT) {
        ...
        Log::add_error("Error: Tile_Counts::get_tile_reference received unknown tile type '" +
                       Strings::num_to_string((uint8_t) tile_type) + "'");
...
Coords<uint32_t> Chunk::get_central_building_coords (const Coords<uint32_t>& chunk_coords) {
    return Coords<uint32_t>(chunk_coords.x * Game_Constants::CHUNK_SIZE + Game_Constants::CHUNK_SIZE / 2 -
                            Game_Constants::BUILDING_SIZE / 2,
                            chunk_coords.y * Game_Constants::CHUNK_SIZE + Game_Constants::CHUNK_SIZE / 2 -
                            Game_Constants::BUILDING_SIZE / 2);
}
...
void Chunk::render_ground (uint32_t chunk_x, uint32_t chunk_y) const {
    ...
    if (Collision::check_rect(box_render * Game_Manager::camera_zoom, Game_Manager::camera)) {
        Render::render_texture(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                               ...
                               Image_Manager::get_image("ground_" + get_ground_string()), 1.0,
...
void Chunk::render_border_overlay (uint32_t chunk_x, uint32_t chunk_y, const string& color) const {
    ...
    if (Collision::check_rect(box_render * Game_Manager::camera_zoom, Game_Manager::camera)) {
        Render::render_rectangle(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                 y * Game_Manager::camera_zoom - Game_Manager::camera.y,
                                 get_size() * Game_Manager::camera_zoom, get_size() * Game_Manager::camera_zoom, 0.2,
                                 color);

        Render::render_rectangle_empty(x * Game_Manager::camera_zoom - Game_Manager::camera.x,
                                       y * Game_Manager::camera_zoom - Game_Manager::camera.y,
                                       get_size() * Game_Manager::camera_zoom, get_size() * Game_Manager::camera_zoom,
                                       1.0, color,
                                       Game_Constants::RENDER_CHUNK_BORDER_THICKNESS * Game_Manager::camera_zoom);
    }
}
