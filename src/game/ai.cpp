...
bool AI_Goal::eat_can_interrupt () const {
    if (!is_in_progress() || is_gather() || is_empty_inventory() || is_eat_at_home() || is_forage() || is_retreat() ||
        is_attack_person_melee() || is_attack_building_melee() || is_build() || is_repair()) {
        return true;
    } else {
        return false;
    }
...
bool AI_Goal::eat_at_home_can_interrupt () const {
    if (!is_in_progress() || is_gather() || is_empty_inventory() || is_retreat() || is_attack_person_melee() ||
        is_attack_building_melee() || is_build() || is_repair()) {
        return true;
    } else {
        return false;
    }
...
bool AI_Goal::forage_can_interrupt () const {
    if (!is_in_progress() || is_gather() || is_empty_inventory() || is_eat_at_home() || is_retreat() ||
        is_attack_person_melee() || is_attack_building_melee() || is_build() || is_repair()) {
        return true;
    } else {
        return false;
...
bool AI_Goal::retreat_can_interrupt () const {
    if (!is_in_progress() || is_gather() || is_forage() || is_attack_person_melee() || is_attack_building_melee() ||
        is_build()) {
        return true;
    } else {
        return false;
...
Tile::Type AI_Goal::get_goal_tile_type () const {
    if (type == Type::GATHER_WHEAT) {
        ...
        // When attacking a building, the goal tile type could actually be either a city building or an unfinished
        // building
...
bool AI_Goal::countdown () {
    if (is_in_progress()) {
        // We check for counter being 0 first, because it is possible for a goal to have a countdown of 0 (i.e.,
        // instant)
