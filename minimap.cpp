...
void Minimap::generate_surface (uint32_t new_width, uint32_t new_height) {
    ...
    chunk_width = ((double) width /
                   double(Game::option_world_width * Game_Constants::CHUNK_SIZE * Game_Constants::TILE_SIZE)) *
                  double(Game_Constants::CHUNK_SIZE * Game_Constants::TILE_SIZE);
    chunk_height = ((double) height /
                    double(Game::option_world_height * Game_Constants::CHUNK_SIZE * Game_Constants::TILE_SIZE)) *
                   double(Game_Constants::CHUNK_SIZE * Game_Constants::TILE_SIZE);

    // TODO What if one or both chunk dimensions are 0?
...
void Minimap::update () {
    if (is_generated()) {
        ...
        if (surface_final != 0) {
            double map_scale_x = (double) width /
                                 double(Game::option_world_width * Game_Constants::CHUNK_SIZE *
                                        Game_Constants::TILE_SIZE);
            double map_scale_y = (double) height /
                                 double(Game::option_world_height * Game_Constants::CHUNK_SIZE *
                                        Game_Constants::TILE_SIZE);
            ...
            for (const auto& it : tiles) {
                if (it.second.is_alive()) {
                    ...
                    if (render_tile) {
                        ...
                        // pixels
                        uint32_t tile_width = ((double) Tile::get_tile_type_size(it.second.get_type()) /
                                               double(Game_Constants::CHUNK_SIZE * Game_Constants::TILE_SIZE)) *
                                              (double) chunk_width;
                        uint32_t tile_height = ((double) Tile::get_tile_type_size(it.second.get_type()) /
                                                double(Game_Constants::CHUNK_SIZE * Game_Constants::TILE_SIZE)) *
                                               (double) chunk_height;
            ...
            // pixels
            uint32_t city_width = (double(Game_Constants::BUILDING_SIZE * Game_Constants::TILE_SIZE) /
                                   double(Game_Constants::CHUNK_SIZE * Game_Constants::TILE_SIZE)) *
                                  (double) chunk_width;
            uint32_t city_height = (double(Game_Constants::BUILDING_SIZE * Game_Constants::TILE_SIZE) /
                                    double(Game_Constants::CHUNK_SIZE * Game_Constants::TILE_SIZE)) *
                                   (double) chunk_height;
                    ...
                    for (uint32_t city_x = x; city_x < x + city_width; city_x++) {
                        for (uint32_t city_y = y; city_y < y + city_height; city_y++) {
                            if (city_x < width && city_y < height) {
                                Pixels::surface_put_pixel(surface_final, city_x, city_y,
                                                          *Object_Manager::get_color(color));
                            }
                        }
                    }
                }
...
Collision_Rect<double> Minimap::get_box () {
    return Collision_Rect<double>((double) Game_Window::width() - (double) width -
                                  Game_Constants::RENDER_MINIMAP_BORDER,
                                  (double) Game_Window::height() - (double) height -
                                  Game_Constants::RENDER_MINIMAP_BORDER, (double) width, (double) height);
}
...
void Minimap::render () {
    ...
    // Render the border
    Render::render_rectangle(box.x - Game_Constants::RENDER_MINIMAP_BORDER,
                             box.y - Game_Constants::RENDER_MINIMAP_BORDER,
                             (double) width + Game_Constants::RENDER_MINIMAP_BORDER * 2.0,
                             (double) height + Game_Constants::RENDER_MINIMAP_BORDER * 2.0, 1.0, "minimap_border");
    ...
    Render::render_rectangle_empty(box.x + (Game_Manager::camera.x * map_scale_x) / Game_Manager::camera_zoom,
                                   ...
                                   (Game_Manager::camera.w * map_scale_x) / Game_Manager::camera_zoom,
                                   (Game_Manager::camera.h * map_scale_y) / Game_Manager::camera_zoom, 1.0,
                                   "minimap_camera_rectangle", Game_Constants::RENDER_MINIMAP_CAMERA_RECTANGLE);
}
