...
void Game::add_remove_objects () {
    ...
    for (auto it = tiles.cbegin(); it != tiles.cend();) {
        if (!it->second.is_alive()) {
            ...
            chunks[Tile::get_chunk_x(it->first.x)][Tile::get_chunk_y(it->first.y)].decrement_tile_count(
                it->second.get_type());
            ...
            if (it->second.get_type() == Tile::Type::BUILDING_UNFINISHED) {
                ...
                civilizations[parent_civilization].remove_unfinished_building(Coords<uint32_t>(it->first.x,
                                                                                               it->first.y));

                if (selection.type == Game_Selection::Type::UNFINISHED_BUILDING &&
                    selection.tile_coordinates == it->first) {
                    clear_selection();
                }
            }
...
void Game::clear_world () {
    ...
    // chunks
    uint32_t MAX_WORLD_SIZE = (uint32_t) numeric_limits<int32_t>::max() /
                              (Game_Constants::CHUNK_SIZE * Game_Constants::TILE_SIZE);
...
void Game::setup_leaders () {
    ...
    for (uint32_t i = 0; i < option_max_leaders; i++) {
        ...
        leaders.back().set_color(Color(rng_color.random_range(0, 255), rng_color.random_range(0, 255),
                                       rng_color.random_range(0, 255), 255));
    }
...
void Game::generate_world () {
    // TODO - Note that I have removed the usual call to clear_world()
    ...
    for (uint32_t x = 0; x < option_world_width; x++) {
        for (uint32_t y = 0; y < option_world_height; y++) {
            ...
            if (!chunks_in_regions[x][y]) {
                ...
                for (uint32_t iteration = 0, distance_from_start = 0; chunks_in_region < region_max && iteration < 100;
                     iteration++) {
                    ...
                    if (rand_direction == "left" && check_x > 0 && !chunks_in_regions[check_x - 1][check_y]) {
                        ...
                        distance_from_start++;
                        chunks_in_region++;
                    } else if (rand_direction == "right" && check_x < option_world_width - 1 &&
                               !chunks_in_regions[check_x + 1][check_y]) {
                        // Add the chunk to the region
                        regions.back().add_chunk(Coords<uint32_t>(check_x + 1, check_y));
                        chunks[check_x + 1][check_y].set_parent_region(regions.size() - 1);
                        chunks_in_regions[check_x + 1][check_y] = true;
                        ...
                        distance_from_start++;
                        chunks_in_region++;
                    } else if (rand_direction == "down" && check_y < option_world_height - 1 &&
                               !chunks_in_regions[check_x][check_y + 1]) {
                        // Add the chunk to the region
                        regions.back().add_chunk(Coords<uint32_t>(check_x, check_y + 1));
                        chunks[check_x][check_y + 1].set_parent_region(regions.size() - 1);
                        chunks_in_regions[check_x][check_y + 1] = true;
    ...
    // pixels
    // We square this value, so it can be directly compared to distances returned by
    // Int_Math::get_distance_between_points_no_sqrt
    ...
    for (uint32_t leader = 0; leader < leaders.size(); leader++) {
        ...
        for (uint32_t attempts = 0;; attempts++) {
            ...
            Coords<int32_t> coords(Tile::get_x(tile_coords.x) + Tile::get_tile_type_size(Tile::Type::BUILDING_CITY) / 2,
                                   Tile::get_y(tile_coords.y) + Tile::get_tile_type_size(Tile::Type::BUILDING_CITY) /
                                   2);
            ...
            if (!get_region(region_index).has_parent_civilization()) {
                if (attempts >= max_attempts || distance_to_nearest_city(coords) >= desired_distance_between_cities) {
                    new_tiles.emplace(std::piecewise_construct, std::forward_as_tuple(tile_coords),
                                      std::forward_as_tuple(city, Tile::Type::BUILDING_CITY));
        ...
        for (uint32_t person = 0; person < Game_Constants::CITY_POPULATION_START; person++) {
            int32_t x = (int32_t) rng.random_range((uint32_t) box_city_spawn_zone.x,
                                                   (uint32_t) (box_city_spawn_zone.x + box_city_spawn_zone.w));
            int32_t y = (int32_t) rng.random_range((uint32_t) box_city_spawn_zone.y,
                                                   (uint32_t) (box_city_spawn_zone.y + box_city_spawn_zone.h));

            new_people.push_back(Person(city,
    ...
    if (leader >= 0) {
        ...
        Game_Manager::center_camera(Collision_Rect<double>((double) city_ptr->get_center_x(),
                                                           (double) city_ptr->get_center_y(), 0.0, 0.0));
    }
...
const Chunk& Game::get_chunk (const Coords<uint32_t>& coords) {
    if (coords.x < chunks.size() && coords.y < chunks[coords.x].size()) {
        ...
        Log::add_error("Error accessing chunk '" + Strings::num_to_string(coords.x) + "," +
                       Strings::num_to_string(coords.y) + "'");
...
const Tile& Game::get_tile (const Coords<uint32_t>& coords) {
    if (tile_exists(coords)) {
        ...
        Log::add_error("Error accessing tile '" + Strings::num_to_string(coords.x) + "," +
                       Strings::num_to_string(coords.y) + "'");
...
void Game::add_leader (bool player_controlled, uint32_t parent_player, const Color& color,
                       const vector<Leader::Diplomacy_State>& diplomacy_states) {
    if (!started) {
...
bool Game::tile_coords_are_valid (Tile::Type type, const Coords<uint32_t>& tile_coords) {
    ...
    if (tile_coords.x + (tile_size - 1) >= get_world_width_tiles() ||
        tile_coords.y + (tile_size - 1) >= get_world_height_tiles()) {
        return false;
    }

    // If the tile is not a building, it is invalid if it is overlapping the building sized rectangle at the center of
    // the chunk
    Collision_Rect<uint32_t> box_tile(tile_coords.x, tile_coords.y, tile_size, tile_size);
    Collision_Rect<uint32_t> box_chunk_center(Tile::get_chunk_x(tile_coords.x) * Game_Constants::CHUNK_SIZE +
                                              Game_Constants::CHUNK_SIZE / 2 - Game_Constants::BUILDING_SIZE / 2,
                                              Tile::get_chunk_y(tile_coords.y) * Game_Constants::CHUNK_SIZE +
                                              Game_Constants::CHUNK_SIZE / 2 - Game_Constants::BUILDING_SIZE / 2,
                                              Game_Constants::BUILDING_SIZE, Game_Constants::BUILDING_SIZE);
    ...
    if (city_list.size() > 1 && !passed_city.has_maximum_population()) {
        ...
        for (size_t i = 0; i < city_list.size(); i++) {
            if (city_list[i] != city_index) {
                ...
                if (city.get_exists()) {
                    city_distance_list.push_back(Game_City_Distance(city_list[i],
                                                                    Int_Math::get_distance_between_points_no_sqrt(
                                                                        Coords<int32_t>(city_x, city_y),
                                                                        Coords<int32_t>(city.get_center_x(),
                                                                                        city.get_center_y()))));
                }
            }
        }
        ...
        if (city_distance_list.size() > 0) {
            ...
            for (size_t i = city_distance_list.size() - 1;;) {
                ...
                if (city.has_excess_population()) {
                    ...
                    for (size_t n = 0, people_moved = 0; n < city_people.size() && people_moved < population_to_move;
                         n++) {
...
vector<Game_City_Distance> Game::get_nearest_city (const Coords<int32_t>& coords) {
    ...
    for (size_t i = 0; i < cities.size(); i++) {
        if (cities[i].get_exists()) {
            uint64_t distance = Int_Math::get_distance_between_points_no_sqrt(coords,
                                                                              Coords<int32_t>(cities[i].get_center_x(),
                                                                                              cities[i].get_center_y()));
...
Coords<uint32_t> Game::get_mouse_coords_tiles () {
    ...
    // tiles
    uint32_t mouse_tile_x = (uint32_t) (mouse_world_x /
                                        ((double) Game_Constants::TILE_SIZE * Game_Manager::camera_zoom));
    uint32_t mouse_tile_y = (uint32_t) (mouse_world_y /
                                        ((double) Game_Constants::TILE_SIZE * Game_Manager::camera_zoom));
...
void Game::center_camera_on_minimap_position () {
    ...
    Collision_Rect<double> box_mouse((double) mouse_x, (double) mouse_y, 1.0, 1.0);
    Collision_Rect<double> box_minimap = minimap.get_box();
    Collision_Rect<double> box_position(Math::abs(box_minimap.x - box_mouse.x) /
                                        (box_minimap.w / (double) get_world_width()),
                                        Math::abs(box_minimap.y - box_mouse.y) /
                                        (box_minimap.h / (double) get_world_height()), 0.0, 0.0);
...
void Game::render () {
    if (started) {
        uint32_t camera_chunk_x = (uint32_t) (Game_Manager::camera.x /
                                              ((double) Game_Constants::CHUNK_SIZE *
                                               (double) Game_Constants::TILE_SIZE * Game_Manager::camera_zoom));
        uint32_t camera_chunk_y = (uint32_t) (Game_Manager::camera.y /
                                              ((double) Game_Constants::CHUNK_SIZE *
                                               (double) Game_Constants::TILE_SIZE * Game_Manager::camera_zoom));
        uint32_t end_chunk_x = camera_chunk_x +
                               (uint32_t) (Game_Manager::camera.w /
                                           ((double) Game_Constants::CHUNK_SIZE * (double) Game_Constants::TILE_SIZE *
                                            Game_Manager::camera_zoom)) + 2;
        uint32_t end_chunk_y = camera_chunk_y +
                               (uint32_t) (Game_Manager::camera.h /
                                           ((double) Game_Constants::CHUNK_SIZE * (double) Game_Constants::TILE_SIZE *
                                            Game_Manager::camera_zoom)) + 2;
        ...
        uint32_t camera_tile_x = (uint32_t) (Game_Manager::camera.x /
                                             ((double) Game_Constants::TILE_SIZE * Game_Manager::camera_zoom));
        uint32_t camera_tile_y = (uint32_t) (Game_Manager::camera.y /
                                             ((double) Game_Constants::TILE_SIZE * Game_Manager::camera_zoom));
        uint32_t end_tile_x = camera_tile_x +
                              (uint32_t) (Game_Manager::camera.w /
                                          ((double) Game_Constants::TILE_SIZE * Game_Manager::camera_zoom)) + 2;
        uint32_t end_tile_y = camera_tile_y +
                              (uint32_t) (Game_Manager::camera.h /
                                          ((double) Game_Constants::TILE_SIZE * Game_Manager::camera_zoom)) + 2;
                ...
                if (x < (uint32_t) get_world_width_tiles() && y < (uint32_t) get_world_height_tiles()) {
                    ...
                    if (tile_exists(tile_coords)) {
                        ...
                        if (selection.type == Game_Selection::Type::CITY) {
                            if (tiles.at(tile_coords).is_alive() &&
                                tiles.at(tile_coords).get_type() == Tile::Type::BUILDING_CITY) {
                            ...
                            if (selection.tile_coordinates == tile_coords) {
                                if (tiles.at(tile_coords).is_alive() &&
                                    tiles.at(tile_coords).get_type() == Tile::Type::BUILDING_UNFINISHED) {
                                    selected = true;
                                }
                            }
                        }
